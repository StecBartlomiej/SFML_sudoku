...
#include "../include/FontID.hpp"

Window::Window(uint32_t width, uint32_t height, const std::string &title) : gridRect_{}, sudoku_{},
                                                                            fontHolder_{}
{
...
void Window::resize(uint32_t width, uint32_t height)
{
    ...
    if (width >= height * 0.8)
    {
        area.reset(sf::FloatRect({0, 0}, {static_cast<float>(width), static_cast<float>(height)}));
    }
    else
    {
        window_.setSize(sf::Vector2u(height * 0.8, height));
        area.reset(sf::FloatRect({0, 0}, {static_cast<float>(window_.getSize().x), static_cast<float>(window_.getSize().y)}));
    }
